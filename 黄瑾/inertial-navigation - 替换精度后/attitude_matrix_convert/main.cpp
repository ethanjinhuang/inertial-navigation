...
void main()
{
	...
	Vector3d BLH(deg2rad(20), deg2rad(30), 10000);
	cout << BLH << endl;
	Vector3d xyz = gcs2gc(BLH);
	cout << xyz << endl;
	BLH = gc2gcs(xyz);
	cout << BLH << endl;
	...
	// m,m-1 次采样delta数据
	Vector3d temp1; temp1 << data.file_data[1].gx, data.file_data[1].gy, data.file_data[1].gz;
	Vector3d temp2; temp2 << data.file_data[2].gx, data.file_data[2].gy, data.file_data[2].gz;
	vector<Vector3d> delta_theta; delta_theta.push_back(temp1); delta_theta.push_back(temp2);
	// 初始化存放m-2,m-1时刻的速度与vector[1], vecotr[0]
	temp1 << 0, 0, 0;
	temp2 << 1, 1, 0;
	vector<Vector3d> speed; speed.push_back(temp1); speed.push_back(temp2);
	// m -1， m-2 时刻的位置数据
	temp1 << -2590726.191, 4469381.673, 3728381.517;	// m-1
	temp2 << -2590726.191, 4469381.673, 3728381.517;	// m-2
	vector<Vector3d> position; position.push_back(temp1), position.push_back(temp2);
	...
	vector<Vector3d> velocity; velocity.push_back(temp2); velocity.push_back(temp1);
