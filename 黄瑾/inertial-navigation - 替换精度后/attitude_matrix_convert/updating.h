...
void update::speed_update()
{
    ...
    */
    ...
    // 计算有害加速度的速度增量
    Vector3d delta_v_ncorg = (-(2 * omega_nie_m_05.value + omega_nen_m_05.value).cross(v_n_m_05) + g_m_05) * inputdata.T;
    ...
    Vector3d delta_v_nsf = (I - inputdata.T / 2 * (omega_nie_m_05 + omega_nen_m_05).EA2ssm()) * attitude_data.Cnb_m.value * (inputdata.Velocity[2] + delta_v_brot + delta_v_bscul);

    // inputdata.T / 2 是否正确？？？？？？

    //计算运载体在导航系下的几何运动速度
    Vector3d v_nm = speed_data[0].v_nem + delta_v_nsf + delta_v_ncorg;
